...
def make_special_bricks(all_bricks, number_of_special_bricks):
    """make some special bricks where ball will be multipled if hit

    Args:
        all_bricks (list of stripe): all bricks
        number_of_special_bricks (int): how many special bricks needed

    Returns:
        list of all bricks and special bricks
    """
...
def main_stage(screen):
    ...
    count = 0
    brick_speed_divisor = cfg.FRAME_DEVISOR_FOR_BRICKS
    ...
    while carryOn:
        ...
        if keys[pygame.K_UP]:
            paddle.moveDown(paddle_speed)
        if keys[pygame.K_DOWN]:
            paddle.moveUp(paddle_speed)
